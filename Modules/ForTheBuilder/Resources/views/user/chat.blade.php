                ...
                <div class="col-xl-3 col-lg-4">
                    <div class="card chat-list-card mb-xl-0">
                        <div class="card-body">
                                ...
                                <div class="flex-shrink-0">
                                    @php
                                        if(!empty($user->avatar)){
                                            $file_url = public_path('/uploads/user/' . $user->id . '/s_' . $user->avatar);
                                        }else{
                                            $file_url = "";
                                        }
                                    @endphp
                                    @if (file_exists($file_url))
                                        <img class="rounded-circle img-thumbnail avatar-md" src="{{ asset('/uploads/user/' . $user->id . '/s_' . $user->avatar) }}" alt="HUman">
                                    @else
                                     @php
                                        $gender_img = 'men.png';
                                        if ($user->gender == 2) {
                                    ...
                                    @endphp
                                        <img class="flex-shrink-0 rounded-circle avatar-sm" src="{{ asset('/backend-assets/img/'.$gender_img) }}" alt="HUman">
                                    @endif
                                </div>
                            ...
                            <div class="">
                                <ul class="list-unstyled chat-list mb-0" style="max-height: 413px;" data-simplebar="init"><div class="simplebar-wrapper" style="margin: 0px;"><div class="simplebar-height-auto-observer-wrapper"><div class="simplebar-height-auto-observer"></div></div><div class="simplebar-mask"><div class="simplebar-offset" style="right: 0px; bottom: 0px;"><div class="simplebar-content-wrapper" tabindex="0" role="region" aria-label="scrollable content" style="height: auto; overflow: hidden scroll;">
                                    <div class="simplebar-content" id="user_list" style="padding: 0px;">
                                    
                                    
                                    </div>
                                </div></div></div><div class="simplebar-placeholder" style="width: auto; height: 519px;"></div></div><div class="simplebar-track simplebar-horizontal" style="visibility: hidden;"><div class="simplebar-scrollbar" style="width: 0px; display: none;"></div></div><div class="simplebar-track simplebar-vertical" style="visibility: visible;"><div class="simplebar-scrollbar" style="height: 328px; transform: translate3d(0px, 0px, 0px); display: block;"></div></div></ul>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="conversation-list-card card">
                        <div class="card-body">
                            ...
                            <div>
                                ...
                                </div></div></div></div><div class="simplebar-placeholder" style="width: auto; height: 606px;"></div></div><div class="simplebar-track simplebar-horizontal" style="visibility: hidden;"><div class="simplebar-scrollbar" style="width: 0px; display: none;"></div></div><div class="simplebar-track simplebar-vertical" style="visibility: visible;"><div class="simplebar-scrollbar" style="height: 277px; transform: translate3d(0px, 132px, 0px); display: block;"></div></div></ul>
                            </div>
                        </div>
...
<script>
    ...
    var conn = new WebSocket('ws://162.55.134.175:1213/?token={{ auth()->user()->token }}');
        ...
        if(data.response_connected_chat_user)
        {
            ...
            if(data.data.length > 0)
            {
                for(var count = 0; count < data.data.length; count++)
                {
                    ...
                    {
                        user_image = `<img src="{{ asset('/backend-assets/img/men.png') }}" class="rounded-circle avatar-sm" />`;
                       
                    }
                    else
                    {
                        user_image = `<img src="{{ asset('uploads/user/') }}/`+data.data[count].id+`/s_`+data.data[count].user_image+`" " class="rounded-circle avatar-sm" />`;
                    }
        ...
        if(data.message)
        {
            ...
            if(data.from_user_id == from_user_id)
            {
                ...
                if(data.sender_connection[0].avatar== null)
                {
                    ...
                    user_image = `<img src="{{ asset('uploads/user/') }}/`+data.from_user_id+`/s_`+data.sender_connection[0].avatar+`" " class="rounded-circle avatar-sm" />`
                }
            ...
            for(var count = 0; count < data.chat_history.length; count++)
            {
                if(data.chat_history[count].user_from_id == from_user_id)
                {
                    ...
                    if(data.sender_connection.avatar== null)
                    {
                        ...
                        user_image = `<img src="{{ asset('uploads/user/') }}/`+data.chat_history[count].user_from_id+`/s_`+data.sender_connection.avatar+`" class="rounded-circle avatar-sm" />`
                    }
                ...
                else
                {
                    ...
                    if(data.receiver_connection.avatar== null)
                    {
                        ...
                        user_image = `<img src="{{ asset('uploads/user/') }}/`+data.chat_history[count].user_from_id+`/s_`+data.receiver_connection.avatar+`" class="rounded-circle avatar-sm" />`
                    }
                    html += `
                    &nbsp; `+user_image+`
                                </div>
        ...
        if(data.group_message)
        {
            ...
            if(data.from_user_id == from_user_id)
            {
                ...
                if(data.sender_connection.avatar== null)
                {
                    ...
                    user_image = `<img src="{{ asset('uploads/user/') }}/`+data.from_user_id+`/s_`+data.sender_connection.avatar+`" class="rounded-circle avatar-sm" />`
                }
            ...
            for(var count = 0; count < data.group_chat_history.length; count++)
            {
                ...
                if(data.group_chat_history[count].user_from_id == from_user_id)
                {
                    ...
                    if(data.group_chat_history[count].avatar== null)
                    {
                        ...
                        user_image = `<img src="{{ asset('uploads/user/') }}/`+data.group_chat_history[count].user_from_id+`/s_`+data.group_chat_history[count].avatar+`"  class="rounded-circle avatar-sm" />`
                    }
