...
<head>
    ...
    <link rel="shortcut icon" type="image/x-icon" href="<?php echo e(asset('/backend-assets/img/favicon1.png')); ?>">
...
<div class="modal fade" id="cancel_confirm_modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="box-shadow: 0 0 10px #000;">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body text-center">
                ...
                <br>
                <div class="d-flex justify-content-center mt-3">
                    <button type="button" class="cancelConfirm btn btn-success"><?php echo e(translate('Confirm')); ?></button>
                </div>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
...
<div id="no_result_text" data-text="<?php echo e(translate('No results found')); ?>"></div>
<div id="permission" data-text="<?php echo e(translate('Permission denied!')); ?>"></div>
    ...
    <script>
        $('.btn-filter[data-filter]').on('click', function() {
            ...
            if (filter == 0) {
                ...
                $('.btn-filter-flat[data-mvd=1]').attr('disabled', false);
                $('.btn-filter-flat[data-mvd=1]').css('opacity', 1);
            }
            else {
        ...
        $(document).on('click', '.flat-button-open-modal', function() {
            $('#exampleModal .modal-body').find('.showMvd').addClass('d-none')
            ...
            if (client != '') {
                ...
                .html('<a href="/clients/show/'+client_id+'/0/0">'+client+'</a>')
            } else {
            ...
            if (status == <?php echo e(Constants::STATUS_SOLD); ?>) {
                ...
                $('#exampleModal .modal-body').find('.showMvd').addClass('d-none')
                $('#exampleModal .modal-body').find('.showBooking').addClass('d-none')
            } else {
            ...
            if (mvd == 1) {
                $('#exampleModal .modal-body').find('.modalSelect').addClass('d-none')
                $('#exampleModal .modal-body').find('.showDetailsStatus').addClass('d-none')
                $('#exampleModal .modal-body').find('.showMvd').removeClass('d-none')
                $('#exampleModal .modal-body').find('.showBooking').addClass('d-none')
            }

            if (status == <?php echo e(Constants::STATUS_BOOKING); ?>) {
                $('#exampleModal .modal-body').find('.modalSelect').addClass('d-none')
                $('#exampleModal .modal-body').find('.showDetailsStatus').addClass('d-none')
                $('#exampleModal .modal-body').find('.showMvd').addClass('d-none')
                if (booking_id > 0) {
                    $('#exampleModal .modal-body').find('.showBooking').attr('href','/booking/show/'+booking_id)
                    $('#exampleModal .modal-body').find('.showBookingDate').text(booking_date)
                }
                $('#exampleModal .modal-body').find('.showBooking').removeClass('d-none')
            }
            else{
                $('#exampleModal .modal-body').find('.showBooking').addClass('d-none')
            }
            ...
            $('#exampleModal .modal-body').find('.modalPodrobnoButton').attr('href',
                '/house-flat/show/' + house_flat_id)
        })

        $(document).on('click','.showBooking',function(){
        ...
        $(document).on('click', '.attach-order', function() {
            ...
            $('#exampleModalNext .modal-body').html(`
                <h4><?php echo e(translate('Oder from which side')); ?></h4>
                <div class="form-check mb-2">
                    <input type="radio" name="order" id="desc" value="1" class="form-check-input" required checked>
        ...
        $(document).on('click', '.selectModalDiv', function() {
            ...
            if (thisVal == <?php echo e(HouseFlat::STATUS_SOLD); ?>) {
                location.replace("/deal/create?house_flat_id=" + house_flat_id +
                    "&house_flat_number=" +
                ...
                console.log(gId)
                $.get('/house/show-more-item-detail/' + gId, function(data) {
                    console.log(data);
                    $.ajax({
                        url: `/house-flat/update-status/${gId}`,
                        type: 'PUT',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
        ...
        $(document).on('click', '#sales', function() {
            ...
            location.replace("/deal/create?house_flat_id=" + gId + "&house_flat_number=" +
        ...
        $(document).on('keyup', '.keyUpName', function(e) {
            ...
            $.get('/house/search-by-name/' + name, function(data) {
                    ...
                    $.each(data['searchedLeadList'], function(index, value) {
                        ...
                        listData +=
                            `<li
                                    ...
                                    given_date="` + moment(value['given_date']).format('DD.MM.YYYY') + `"
        ...
        $(document).on('change', '.deal_create_house_id', function() {
            ...
            if (houseID) {
                $.ajax({
                    type: "GET",
                    datatype: "json",
                    url: "/deal/get-flat?house_id=" + houseID,
                    success: function(data) {
                        // console.log(data)
                        if (data) {
        ...
        $(document).on('click', '#customckeck1_house', function(e) {
            if ($(this).prop("checked") == true) {
                ...
                // $('#discount_sum_words').prop('disabled',true)
                $('#discount').addClass('bg-light')
                // $('#discount_sum_words').addClass('bg-light')
                $('#discount').val('')
                // $('#discount_sum_words').val('')
                $('#initial_fee_words').val('')
                $('#discount_sum').val('')
                $('#discount_sum_words').val('')
                ...
                // $('#discount_sum_words').prop('disabled',false)
                $('#discount').val('')
                // $('#discount_sum_words').val('')
                $('#discount').removeClass('bg-light')
                // $('#discount_sum_words').removeClass('bg-light')
                $('.initialFeeDeal').val('')
                $('#initial_fee_words').val('')
                $('#initial_fee_discount').val('')
                $("#period").val(null).trigger("change"); 
                $("#org_period").val('') 
                $("#percent").val(null).trigger("change"); 
                $("#org_percent").val(''); 
        ...
        //         var setVal = parseInt(parseInt(dealCreatePrice) / 100 * parseInt(percent))
                
        //         $('.initialFeeDeal').val(Math.ceil(setVal))

        //     }
        //     else if(thisVal == 0){
        //         var dealCreatePrice = $('.dealCreatePrice').val()
        //         var period = $("#period").select2().find(":selected").data("id");

        //         if(period != ''){
                    
        //             var setVal = parseFloat(dealCreatePrice) / parseFloat(period)
        //             $('.initialFeeDeal').val(Math.ceil(setVal))
        //         }
        //         else{
                    
        //             $('.initialFeeDeal').val(Math.ceil(dealCreatePrice))   
        //         }
        //     }
        // })
        ...
        $(document).on('keyup', '#coupon', function(e) {
            ...
            $('#applied').text('')
            $.ajax({
                url: `/house-flat/search-coupon/${text}`,
                // data: {status: 0},
                type: 'GET',
                success: function(data) {
                    if (!(data.length === 0)) {
        ...
        $(document).on('change', '.priceInformationSelectHouse', function(e) {
            ...
            $('.priceFormationOpenFlats2').attr('data-bs-toggle', 'modal').attr('data-bs-target', '#exampleModal')
        })
        ...
        $(document).on('click', '.priceFormationOpenFlats2', function(e) {
            $('#exampleModal .modal-body').html('')
            arrPriceFormation = new Array();
        ...
        $(document).on('click', '.savePriceFormation', function(e) {
            ...
            $('.priceFormationOpenFlats2').css('opacity', 1)
            $('.priceFormationOpenFlats2').text("<?php echo e(translate('Flat choosen')); ?>")
        })
        ...
        $(document).on('click', '.saveDealDogovor', function(e) {
            ...
            if($('#customckeck1_house').is(':checked') && $('#period').val() == ''){
                $('#period_error').css('display','block')
                $('#period_error').siblings('span').find('.select2-selection--single').css('border-color','red')
            ...
            // if($('#discount').val() != '' && $('#customckeck1_house').not(':checked')){
            //     $('#exampleModalSave').modal('toggle')
            // }
        })
        ...
        $(document).on('click','.yes_now',function(){
            ...
            var client_id = $('.booking-client_id').val()
            var phone = $('#phone').val()
            if (phone) {
                $.ajax({
                    url: `/deal/validate-series-number`,
                    data: {
                    ...
                    success: function(data) {
                        if (data.status == 'success') {
                            ...
                            $('#exampleModalSave').modal('toggle')
                            $('#series_number').addClass('is-invalid')
                        }
                    },
                    error: function(data) {
                        console.log(data);
        ...
        var permission_text = $('#permission').attr('data-text')
        // kanban
        "use strict";
        !(function (e) {
            function o() {
            ...
            e("#upcoming_1, #upcoming_2, #upcoming_3, #upcoming_4")
                .sortable({
                    ...
                    remove: function( event, ui ) {
                        ...
                        const role = ui.item[0].dataset.role
                        ...
                        if (role != 4) {
                            ...
                            $("#upcoming_1, #upcoming_2, #upcoming_3, #upcoming_4").sortable( "cancel");
                            showToast('danger', permission_text)
                        }
                    ...
                    remove: function( event, ui ) {
                        ...
                        if (role != 4) {
                            if (type == 3) {
                                ...
                                $('#cancel_textare').val('')
                                $('#cancel_confirm_modal').modal('toggle')
                                $('#cancel_confirm_modal .cancelConfirm').attr('data-id', id)
                                $('#cancel_confirm_modal .cancelConfirm').attr('data-parent', parent)
                            }
                            else if(type == 2 && parent == 'deal_1'){
                                $.ajax({
                                    type: "GET",
                                    datatype: "json",
                                    url: `/deal/update-deal/`,
                                    data:{
                                        id: id,
                                        parent: parent
                                    },
        ...
        const no_result_text = $('#no_result_text').attr('data-text') 
        $('.select2').select2({
        ...
        $('#exampleFormControlSelect1').select2({
            ...
            if (id == 2) {
                $('#add_flat').removeClass('d-none')
            }
            else{
                $('#add_flat').addClass('d-none')   
            }    
        });
        ...
        $(document).on('click','.cancelConfirm',function(){
            ...
            var id = $(this).attr('data-id')
            var new_parent = $(this).attr('data-parent')

            if (text != '' && id != '' && new_parent != '') {
                $.ajax({
...
<?php /**PATH /Users/user/Desktop/laravel/icstroy/Modules/ForTheBuilder/Resources/views/layouts/forthebuilder.blade.php ENDPATH**/ ?>