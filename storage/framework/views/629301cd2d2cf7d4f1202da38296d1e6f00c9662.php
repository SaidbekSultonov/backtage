<?php $__env->startSection('styles'); ?>
    ...
    <link rel="stylesheet" href="<?php echo e(asset('/backend-assets/plugins/ekko-lightbox/ekko-lightbox.css')); ?>">
...
<style>
    ...
    .floor-action, .floor-marge-action{
        display: none;
    }
</style>
    <script defer src="<?php echo e(asset('/backend-assets/plugins/filterizr/jquery.filterizr.min.js')); ?>"></script>
    <script defer src="<?php echo e(asset('/backend-assets/plugins/ekko-lightbox/ekko-lightbox.min.js')); ?>"></script>
    <script defer src="<?php echo e(asset('/backend-assets/plugins/toastr/toastr.min.js')); ?>"></script>
    ...
    <script defer>
        $(document).ready(function() {
            ...
            $('.house-box').on('click', function(e) {
                ...
                $.get('/forthebuilder/house/show-more-item-detail/' + id, function(data) {
                    ...
                    $('#info1_flat').append(
                        `<div><span><?php echo e(__('locale.Number of flat')); ?>: </span><b>${data.flatItemDetail.number_of_flat}</b></div>`
                    )
                    $('#info1_flat').append(`<div><span><?php echo e(__('locale.Flat price')); ?>: </span><b>${(Math.round(data.flatItemDetail.total_area) * data.flatItemDetail.price).toLocaleString('en-US', {
                        style: "currency",
                        currency: "USD" ,
                    })}</b> $</div>`)
                    $('#info2_flat div').remove()
                    $('#info2_flat').append(
                        `<div><span><?php echo e(__('locale.Total area')); ?>: </span><b>${data.flatItemDetail.total_area}</b> m<sup>2</sup></div>`
                    )
                    $('#info2_flat').append(
                        `<div><span><?php echo e(__('locale.Room of flat')); ?>: </span><b>${data.flatItemDetail.room_count}</b></div>`
                    )
                    ...
                    $('#flatItemDetailShow').append(
                        `<a href='/forthebuilder/house-flat/show/${id}' class='style-add btn btn-primary' style='color:#fff'><?php echo e(__('locale.show')); ?></a>`
                    );
            ...
            function select_status(val) {
                $.get('/forthebuilder/house/show-more-item-detail/' + gId, function(data) {
                    if (data['flatItemDetail']['status'] == 0) {
                        ...
                        switch (val) {
                            case '2':
                                // location.replace("/forthebuilder/deal/create?house_flat_id=" + gId + '&name=' + number_of_flat + '&house_id=' + '<?php echo e($model->id); ?>' + '&house_name=' +'<?php echo e($model->house_number); ?>'  + '&contract_number=' + contract_number);
                                location.replace("/forthebuilder/deal/create?house_flat_id=" + gId +
                                    "&house_flat_number=" + data.flatItemDetail.number_of_flat +
                                    "&house_id=" + '<?php echo e($model->id); ?>' + '&house_name=' +
                                    '<?php echo e($model->house_number); ?>' + '&contract_number=' + data
                        ...
                        switch (val) {
                            ...
                            case '1':
                                toastr.warning("<?php echo e(__('locale.the apartment is already booked')); ?>");
                                break;
                            case '2':
                                // location.replace("/forthebuilder/deal/create?house_flat_id=" + gId + '&name=' + number_of_flat + '&house_id=' + '<?php echo e($model->id); ?>' + '&house_name=' +'<?php echo e($model->house_number); ?>'  + '&contract_number=' + contract_number);
                                location.replace("/forthebuilder/deal/create?house_flat_id=" + gId +
                        ...
                        switch (val) {
                            case '1':
                                toastr.warning("<?php echo e(__('locale.the apartment is already sold')); ?>");
                                break;
                            case '2':
                                // location.replace("/forthebuilder/deal/create?house_flat_id=" + gId + '&name=' + number_of_flat + '&house_id=' + '<?php echo e($model->id); ?>' + '&house_name=' +'<?php echo e($model->house_number); ?>'  + '&contract_number=' + contract_number);
                                location.replace("/forthebuilder/deal/create?house_flat_id=" + gId +
        ...
        $(document).on('mouseenter', '.hover-remove-add', function() {
            $(this).find('.floor-action').fadeIn()
        })

        $(document).on('mouseleave', '.hover-remove-add', function() {
            $(this).find('.floor-action').fadeOut()
        })
        ...
        $(document).on('click', '.basket-to-house', function() {
            ...
            if (from != '') {
                $.ajax({
                    ...
                    type: 'POST',
                    success: function(data) {
                        location.replace("/house/show-more/" + data)
                    }
                })    
            }
            else{
                $('#exampleModalNext .modal-body').addClass('needs-validation was-validated')
            }
        ...
        $(document).on('click', '.bascket-float-remove', function() {
            ...
            $.ajax({
                url: `/house/remove-bascket-float`,
                data: {
                    house_id: house_id,
                    entrance: entrance,
                    floor: floor,
...
${price} у.е.
                        ...
                        <br>
                        <div class="form-control" style="border: 1px solid grey;">
                            <?php echo e(translate('Number of rooms')); ?>
...
${rooms}
                        ...
                        <br>
                        <button class="form-control open-statuses"><?php echo e(translate('Status')); ?></button>
                        ...
                        <br>
                        <a href="/forthebuilder/house-flat/show/${fId}" type="button" class="modalPodrobnoButton btn btn-primary"><?php echo e(translate('Detail')); ?></a>
        ...
        function select_status(val, fId) {
            $.get('/house/show-more-item-detail/' + fId, function(data) {
                if (data['flatItemDetail']['status'] == 0) {
                    switch (val) {
                        case '2':
                            location.replace("/deal/create?house_flat_id=" + fId +
                                "&house_flat_number=" + data.flatItemDetail.number_of_flat +
                                "&house_id=" + '<?php echo e($model->id); ?>' + '&house_name=' +
                                '<?php echo e($model->house_number); ?>' + '&contract_number=' + data
                                    .flatItemDetail.contract_number);
    ...
    </script><?php /**PATH /Users/user/Desktop/laravel/icstroy/Modules/ForTheBuilder/Resources/views/house/extra.blade.php ENDPATH**/ ?>